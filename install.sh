...
echo -e "
${BLUE}
 ___  ___      ___    ___ ________  ________  ___       ________  ________   ___  ________  ___  __    _______   ________     
|\  \|\  \    |\  \  /  /|\   __  \|\   __  \|\  \     |\   __  \|\   ___  \|\  \|\   __  \|\  \|\  \ |\  ___ \ |\   __  \    
\ \  \\\  \   \ \  \/  / | \  \|\  \ \  \|\  \ \  \    \ \  \|\  \ \  \\ \  \ \  \ \  \|\  \ \  \/  /|\ \   __/|\ \  \|\  \   
 \ \   __  \   \ \    / / \ \   ____\ \   _  _\ \  \    \ \   __  \ \  \\ \  \ \  \ \   __  \ \   ___  \ \  \_|/_\ \   __  \  
  \ \  \ \  \   \/  /  /   \ \  \___|\ \  \\  \\ \  \____\ \  \ \  \ \  \\ \  \ \  \ \  \ \  \ \  \\ \  \ \  \_|\ \ \  \ \  \ 
   \ \__\ \__\__/  / /      \ \__\    \ \__\\ _\\ \_______\ \__\ \__\ \__\\ \__\ \__\ \__\ \__\ \__\\ \__\ \_______\ \__\ \__\
    \|__|\|__|\___/ /        \|__|     \|__|\|__|\|_______|\|__|\|__|\|__| \|__|\|__|\|__|\|__|\|__| \|__|\|_______|\|__|\|__|
             \|___|/                                                                                                          
${NC}

${GREEN}A clean and simple Hyprland theme${NC}
...
PACKAGES=(
    ...
    dunst fish geany gvfs kitty kvantum kvantum-qt5 libnotify mako 
...
usage() {
    ...
    echo "  --uninstall   Undo all changes made by installhyprland.sh"
    exit 1
}
...
check_for_update() {
    if [[ -f "$CLONE_DIR/installhyprland.sh" ]]; then
        log_info "[+] Checking for newer installhyprland.sh..."
        local latest_script
        latest_script=$(mktemp)
        # Since this is a new script, we'll skip the update check initially
        log_info "[+] New installhyprland.sh script - no update check needed."
        rm "$latest_script"
    fi
}
...
copy_dotfiles() {
    ...
    if (( DRYRUN )); then
        ...
        # Ensure wlogout configuration files are properly copied
        if [[ -d "$CONFIG_SOURCE/wlogout" ]]; then
            mkdir -p "$CONFIG_TARGET/wlogout"
            cp -f "$CONFIG_SOURCE/wlogout/"* "$CONFIG_TARGET/wlogout/" 
            log_success "[+] wlogout configuration files copied successfully."
        fi
    fi
}
...
setup_theming() {
    ...
    if (( DRYRUN )); then
        ...
        if [[ -n "$kvantum_theme" ]]; then
            DRYRUN_SUMMARY+=("Would apply $kvantum theme in Kvantum")
        else
...
QT6CT_EOF
        ...
        # Also configure qt5ct if available
        if command -v qt5ct &>/dev/null; then
            mkdir -p "$HOME/.config/qt5ct"
            cat > "$HOME/.config/qt5ct/qt5ct.conf" << 'QT5CT_EOF'
[Appearance]
custom_palette=false
standard_dialogs=default
style=kvantum
...
QT5CT_EOF
        ...
        log_success "[+] Applied Qt6 theme and icons via qt6ct/qt5ct."
        ...
        sed -i '/# Hyprland-Laniakea Theme Settings/d' "$profile_file" 2>/dev/null || true
        ...
        {
            echo ""
            echo "# Hyprland-Laniakea Theme Settings"
        ...
        if [[ -f "$bashrc_file" ]]; then
            sed -i '/# Hyprland-Laniakea Theme Settings/d' "$bashrc_file" 2>/dev/null || true
...
uninstall() {
    log_info "[+] Uninstalling hyprland-laniakea setup..."
    if (( DRYRUN )); then
        DRYRUN_SUMMARY+=("Would uninstall hyprland-laniakea-theme, restore backup, remove all installed packages and dotfiles")
    else
        # Remove dotfiles
        rm -rf "$CONFIG_TARGET"
